...
func (pr PaperRepository) GetExamResult(id string, resid string) (res []map[string]interface{}, err error) {
	...
	match2 := bson.M{"$match": bson.M{"results._id": _resid}}
	unwind2 := bson.M{"$unwind": "$questions"}
	unwind3 := bson.M{"$unwind": "$results.selected"}
	redact := bson.M{"$redact": bson.M{"$cond": bson.A{bson.M{"$eq": bson.A{"$questions.qstID", "$results.selected.qstID"}}, "$$KEEP", "$$PRUNE"}}}
	addFields := bson.M{"$addFields": bson.M{"questions.selected": "$results.selected.option", "result_id": "$results._id", "student_id": "$results.student_id"}}
	project := bson.M{"$project": bson.M{"results": 0}}
	group := bson.M{"$group": bson.M{"_id": bson.M{"_id": "$_id", "title": "$title", "subject": "$subject", "teacher_id": "$teacher_id", "student_id": "$student_id", "startFrom": "$startFrom", "endAt": "$endAt", "result_id": "$result_id"}, "result": bson.M{"$push": "$questions"}}}
	project2 := bson.M{"$project": bson.M{"_id": 0, "detail": "$_id", "result": 1}}
	...
	curs, err := pr.db.Collection("paper").Aggregate(pr.ctx, bson.A{match, unwind, match2, unwind2, unwind3, redact, addFields, project, group, project2, unwind4, group2, addFields2})
	if err != nil {
		return nil, err
	}
	err = curs.All(pr.ctx, &res)
